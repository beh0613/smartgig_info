...
const videoAssets = {
  1: `${BASE_URL}videos/1.mp4`,
  2: `${BASE_URL}videos/2.mp4`,
  3: `${BASE_URL}videos/3.mp4`,
  4: `${BASE_URL}videos/4.mp4`,
  5: `${BASE_URL}videos/5.mp4`,
  6: `${BASE_URL}videos/6.mp4`,
  7: `${BASE_URL}videos/7.mp4`,
  "8_default": `${BASE_URL}videos/8_1.mp4`,
  "8_personalized": `${BASE_URL}videos/8_2.mp4`,
  9: `${BASE_URL}videos/9.mp4`,
};
  ...
  const steps = [
    {
      ...
      highlights: ["Personal details form", "Fill account password", "Basic setup completed"]
    },
    {
      id: 2,
      badge: "Step 02",
    ...
    },
    {
      ...
      highlights: ["Google Single Sign-On",  "Navigate to driver portal"]
    },
    {
      id: 5,
      badge: "Step 05",
    ...
    <div className={styles.stickyTerminalFrame}>
      ...
      <div className={styles.terminalHeaderHeader}>
        ...
        <span className={styles.terminalHeaderText}>
           STEP_{currentStep.id < 10 ? `0${currentStep.id}` : currentStep.id}
        </span>
      </div>
      ...
      <div className={styles.terminalBodyContent}>
        {currentStep.id === 8 && (
          <div className={styles.videoModeToggleBar}>
            <button 
              ...
              onClick={(e) => { e.stopPropagation(); setStep8VideoMode("default"); }}
            >
              📊 Default
            </button>
            <button 
              className={`${styles.toggleModeBtn} ${step8VideoMode === 'personalized' ? styles.toggleModeBtnActive : ''}`}
              onClick={(e) => { e.stopPropagation(); setStep8VideoMode("personalized"); }}
            >
              ⚙️ Personalized
            </button>
          </div>
        ...
        <div className={styles.videoPlayerContainer}>
          {/* CRITICAL FIX: Moving src attribute directly onto the main video tag with a URL key 
            forces the underlying browser stream parser to update tracks instantly on re-renders!
          */}
          <video 
            key={`${currentStep.id}_${step8VideoMode}`}
            src={currentVideoSrc}
            ...
            playsInline
          >
            Your browser doesn't support built-in MP4 playback features.
          </video>
          <div className={styles.videoOverlayGlow}></div>
        </div>

        <span className={styles.displayBadgeBadge}>
          {currentStep.id === 8 ? `Step 08 — ${step8VideoMode.toUpperCase()}` : currentStep.badge}
        </span>
    ...
    <div className={styles.pageContainer}>
      ...
      <div className={styles.wideSection}>
        <div className={styles.titleContainer}>
          ...
          <p className={styles.cyberSubtitle}>Follow our simple step-by-step process timeline to set up your account, personalize parameters, and preview ride safety reports.</p>
          <div className={styles.titleLine}></div>
        </div>

        <div className={styles.interactiveWorkflowShell}>
          
          {/* LEFT COLUMN: ACTIVE STEP TIMELINE TILES */}
          <div className={styles.timelineSelectionColumn}>
            <div className={styles.verticalTimelineTreeLine}></div>
            
            {steps.map((step, idx) => {
              const isSelfActive = activeStep === idx;
              return (
                <div key={step.id} style={{ display: 'contents' }}>
                  <div 
                    className={`${styles.timelineButtonCard} ${isSelfActive ? styles.cardActiveState : ''}`}
                    onClick={() => setActiveTabStep(idx)}
                  >
                    <div className={`${styles.timelineBubbleNode} ${isSelfActive ? styles.bubbleNodeActive : ''}`}>
                  ...
                  {/* MOBILE VIEW DRAWER ANCHOR */}
                  {isSelfActive && (
                    <div className={styles.mobileTerminalWrapper}>
          ...
          {/* DESKTOP STICKY CONTROL SCREEN MONITOR PANEL */}
          <div className={styles.desktopTerminalColumn}>
            {renderTerminalContent()}
          </div>
